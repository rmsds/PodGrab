...
# Version 1.1.5 - 8/14/2015
#    - fixed bug in OPML reader that would input a feed with no href, making an entry that stopped processing and could not be removed.
#    - converted from xml.dom.minidom to feedparser
# David Smith
...
import feedparser       # Feedparser is a non-standard library, see https://pypi.python.org/pypi/feedparser and https://pythonhosted.org/feedparser for information
import datetime
...
# Database Check/Create
    if does_database_exist(current_directory):
        db_connection = connect_database(current_directory)
        ...
        if not db_connection:
            ...
            db_cursor = db_connection.cursor()
            setup_database(db_cursor, db_connection)
...
# Main execution
    if not has_error:
        if mode == MODE_UNSUBSCRIBE:
            feed_name = get_name_from_feed(db_cursor, db_connection, feed_url)
            if feed_name == "None":
                ...
                delete_subscription(db_cursor, db_connection, feed_url)
                try :
            ...
            list_subscriptions(db_cursor, db_connection)
        elif mode == MODE_UPDATE:
            print("Updating all podcast subscriptions...")
            subs = get_subscriptions(db_cursor, db_connection)
            for sub in subs:
                feed_name = sub[0]
                feed_url = sub[1]
                ...
                if not data:
                    ...
                    message = iterate_feed(data, mode, DOWNLOAD_DIRECTORY, todays_date, db_cursor, db_connection, feed_url)
                    print(message)
                    mail += message
            mail = mail + "\n\n" + str(total_items) + " podcasts totalling " + str(total_size) + " bytes have been downloaded."
            if has_mail_users(db_cursor, db_connection):
                print("Have e-mail address(es) - attempting e-mail...")
                mail_updates(db_cursor, db_connection, mail, str(total_items))
        elif mode == MODE_DOWNLOAD or mode == MODE_SUBSCRIBE:
            print(iterate_feed(data, mode, DOWNLOAD_DIRECTORY, todays_date, db_cursor, db_connection, feed_url))
        elif mode == MODE_MAIL_ADD:
            add_mail_user(db_cursor, db_connection, mail_address)
            print("E-Mail address: " + mail_address + " has been added")
        elif mode == MODE_MAIL_DELETE:
            delete_mail_user(db_cursor, db_connection, mail_address)
            print("E-Mail address: " + mailAddress + " has been deleted")
        elif mode == MODE_MAIL_LIST:
            list_mail_addresses(db_cursor, db_connection)
        elif mode == MODE_EXPORT:
            export_opml_file(db_cursor, db_connection, current_directory)
        elif mode == MODE_IMPORT:
            import_opml_file(db_cursor, db_connection, current_directory, DOWNLOAD_DIRECTORY, import_file_name)
    else:
...
def iterate_feed(data, mode, download_dir, today, cur, conn, feed):
    ...
    message = ""
    try:
        xml_data = feedparser.parse(data)
        #if f.bozo:
            #raise f.bozo_exception
        channel_title = xml_data.feed.title
        channel_link = xml_data.feed.link
        ...
        if mode == MODE_DOWNLOAD:
            ...
            num_podcasts = iterate_channel(xml_data, today, mode, cur, conn, feed, channel_title)
        ...
        print("ERROR - Malformed XML syntax in feed. Skipping...")
        print("ERROR - " + str(e))
        message += "0 podcasts have been found from this feed due to RSS syntax problems. Please try again later"
    return message
...
def iterate_channel(chan, today, mode, cur, conn, feed, channel_title):
    ...
    if does_sub_exist(cur, conn, feed):
        ...
        insert_subscription(cur, conn, chan.feed.title, feed)
    ...
    for item in chan.entries:
        try:
            item_title = item.title
            item_desc = item.description
            item_date = item.published
            struct_time_item = item.published_parsed
            
            item_file = item.enclosures[0].href
            item_size = item.enclosures[0].length
            item_type = item.enclosures[0].type
